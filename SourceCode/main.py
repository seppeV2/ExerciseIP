...
from SourceCode.network_summary import create_summary
from SourceCode.ownFunctions import makeOwnToyNetwork, getODGraph
from SourceCode.dyntapy_green_time_change import GreenStaticAssignment
...
def main(demand, methodCost, methodGreen):
    ...
    g_bis = build_network(g)
    ff_tt = g_bis.links.length / g_bis.links.free_speed
    ff_tt = [ff_tt[0], 0.0001, ff_tt[2], 2*ff_tt[3]]
    [result, ff_tt, dos] = assignment.run_greens('msa', firstGreen,methodCost,g, ff_tt)
    ...
    while gap > delta and safety < maxLoops:
        ...
        newResult, ff_tt, dos = assignment.run_greens('msa', greens,methodCost,g, ff_tt)
...
def start():
    ...
    demand = 30
    main(demand, methodCost, methodGreen)
